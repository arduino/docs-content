...
Start receiving IMU readings with `imu.begin();` and start the display with `Display.begin();`. Then we can assign attributes to the images such as its source, alignemnt and how the rotation should behave. For more information on image attributes with lvgl, check out our [lvgl tutorial.](tutorials/lvgl-guide#image)
