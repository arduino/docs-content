...
In addition to these functionalities, both Opta™ are connected to the Arduino IoT Cloud via Wi-Fi® to leverage monitored functionalities and remote control.
...
The following headers are required to enable the Modbus RTU protocol, the connection with the Arduino Cloud, and the scheduler. The scheduler will handle the data exchange using Modbus RTU protocol while prioritizing the `loop()` to manage the local tasks of the Small Tank. Here are also defined the parameters needed per Modbus RTU specification as `preDelay` and `postDelay`.
...
The following code structure is used whenever an Opta™ exchanges information with another Opta™ using Modbus RTU protocol. This method writes the value data in the input register for addressee Opta™ defined by means of the specified Modbus address.
...
Within Arduino Cloud's dashboard, the system status of both the tanks can be monitored and the remote actuation is implemented for both the Opta™ devices' managed tasks. By means of this powerful tool, the actuators and emergency stop can be controlled remotely on-demand. The dashboard can also be used to make a simulation, even in absence of the full implementation of the application note.
