...
***It's recommended to use USB memory sticks with a storage capacity of 256 GB or less. If a memory stick has a storage capacity greater than 256 GB, it is advised to partition it to limit its accessible storage to 256 GB or less.***
...
***By default, the `Arduino_UnifiedStorage` library is installed with the `Arduino_POSIXStorage` and `Arduino_USBHostMbed5` libraries by the Arduino IDE's Library Manager. Check that all of the libraries are installed correctly.***
