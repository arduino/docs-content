...
description: 'Learn how to connect to Wi-Fi® networks, how to make an HTTP request to a REST API.'
---
...
- how to convert the response into a readable format using JSON.
...
Connecting to Wi-Fi® is a pretty straightforward concept, and is very similar to how we connect using our phones/computers.
...
Then, in the `boot.py` file, we can paste the Wi-Fi connection code that we just previously.
...
In this exercise, we will make a request to something called a network time protocol (NTP) server. 

As our board does not know the current time, it is necessary for it to make a request to a server that keeps track of it.

Open the code editor, and copy and paste the following script to the `main.py` file:
