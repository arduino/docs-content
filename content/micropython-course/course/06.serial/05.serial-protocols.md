...
- What I2C, SPI, and UART are, and a brief introduction to how they work,
...
Serial protocols are a collective term for techniques that send data electronically, bit by bit in a **serial** fashion. We have previously learned about digital signals, and how they have only two states: 1 (HIGH) and 0 (LOW). 
...
Now that was just a very rough idea of how they work. In reality, it is much more complex. Fortunately, we have **serial protocols** that help us to send and receive this data in a much easier fashion!

With some fundamental knowledge of this, we can now move on to learn about the I2C, SPI and UART protocols. All protocols mentioned in this article have bidirectional communication support, meaning that data can be sent in either direction.
...
For example, we can connect a sensor, and a display to the same pins, read the sensor data, and write the value to the display.
...
The COPI/CIPO is used to transfer the data, while SCK is used to synchronize the data transmission.

Finally, the chip select pin is used to determine whether the device should communicate with a controller or ignore it.

### Universal Asynchronous Receiver-Transmitter (UART)

UART is one of the oldest and historically most used serial protocols. With a UART controller, we have two lines (TX/RX), which are used to connect two devices. 

Bits of information are then sent sequentially from one device to another, where the receiving end reconstructs the bits into complete bytes.

UART is ideal for one-to-one communication between two devices and requires only two wires and no additional clock to function. It is significantly slower than SPI but is rather easy to set up and use, and of course, uses fewer wires.
...
Serial protocols are an advanced topic, and in this chapter, we have only covered a fraction, so that you have an idea of how the examples work.
