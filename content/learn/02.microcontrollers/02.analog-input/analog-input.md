...
The `analogRead()` function will not work correctly if a pin has been previously set to an output, so if this is the case, set it back to an input before using `analogRead()`. Note that calling `pinMode(pin, INPUT)` will disable the internal pull-up resistor, regardless of the pin's previous state. If you need the pull-up resistor enabled while reading analog values, use `pinMode(pin, INPUT_PULLUP)` instead.
