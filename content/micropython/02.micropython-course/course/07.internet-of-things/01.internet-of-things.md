...
# NTP timestamp starts from 1900. 
# Embedded MicroPython ports (like ESP32) use an epoch of 2000-01-01.
# The difference in seconds between 1900 and 2000 is 3155673600.
...
# Unpack the data and extract timestamp (seconds since 1900)
ntp_time = ustruct.unpack("!I", data[40:44])[0]

# Convert to MicroPython timestamp (seconds since 2000)
mpy_time = ntp_time - NTP_DELTA

# Get local time tuple
t = utime.localtime(mpy_time)

# Print time with zero-padded formatting
print("Year:%d Month:%d Day:%d Time: %02d:%02d:%02d" % (t[0], t[1], t[2], t[3], t[4], t[5]))
...
2. The response comes in a Year/Month/Day/Hour/Minute/Second format, and is printed in the terminal with proper zero-padding for time values. 
